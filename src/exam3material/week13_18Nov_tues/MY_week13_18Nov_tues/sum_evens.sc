...
def sumEvens(n: Z): Z = {
  ...
  while (cur != n) {
    ...
    //need a deduce block
    //premises: cur == Old(cur) + 1, sum == Old(cur) * (Old(cur) + 1), cur != n, n >= 0
    //need: statement about the invariant whithout an Old
      //(Won't be able to prove the exact invariant yet)

    sum = sum + 2*cur

    //premises: sum == Old(sum) + 2*cur, n >= 0
    //need: sum == cur * (cur + 1)
  }
